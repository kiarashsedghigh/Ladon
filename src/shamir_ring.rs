...
mod tests {
    ...
    fn sample_ring(seed: u32) -> Ring {
        // Deterministic non-trivial Ring in coefficient form for testing.
        let mut r = Ring::ZEROES_DEGREE255;
        for i in 0..256 {
            r.data[i] = ((seed.wrapping_mul(2654435761).wrapping_add(i as u32 * 40503))
                % params::Q) as u32;
        }
        r
    }
    ...
    #[test]
    fn test_share_reconstruct_ring() {
        let secret = sample_ring(7);
        let (t, n) = (2usize, 5usize);
        let shares = share_ring(&secret, t, n);
        assert_eq!(shares.len(), n);

        // Any t+1 = 3 shares reconstruct the ring exactly.
        let subset: Vec<RingShare> = shares[0..t + 1].to_vec();
        let recon = reconstruct_ring(&subset);
        assert_eq!(recon, secret);

        // A different 3-subset also reconstructs.
        let subset2: Vec<RingShare> = shares[2..5].to_vec();
        assert_eq!(reconstruct_ring(&subset2), secret);
    }
    ...
    fn test_share_reconstruct_vector() {
        ...
        for k in 0..K {
            secret.data[k] = sample_ring(100 + k as u32);
        }

        let (t, n) = (3usize, 7usize);
        let shares = share_vector::<K>(&secret, t, n);
        assert_eq!(shares.len(), n);

        // All eval points distinct and equal to 1..=n.
    ...
    #[test]
    fn test_share_preserves_ntt_tag() {
        // Sharing an NTT-form ring must yield NTT-tagged shares (so parties can
        // run Ring::mult during decryption), and reconstruction must recover it.
        let mut secret = sample_ring(13);
        secret.t = RingRepresentation::NTT; // treat coeffs as NTT-domain values
        let (t, n) = (2usize, 5usize);
        let shares = share_ring(&secret, t, n);

        for s in &shares {
            assert_eq!(s.ring.t, RingRepresentation::NTT);
        }

        let subset: Vec<RingShare> = shares[0..t + 1].to_vec();
        let recon = reconstruct_ring(&subset);
        assert_eq!(recon.t, RingRepresentation::NTT);
        assert_eq!(recon, secret);
    }
}