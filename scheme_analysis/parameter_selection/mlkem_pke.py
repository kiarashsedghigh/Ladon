"""
ML-KEM key and ciphertext size computation.

This module computes the key and ciphertext sizes for ML-KEM (Module-Lattice-Based
Key-Encapsulation Mechanism) with and without compression, based on scheme parameters.
...
def compute_mlkem_key_ciphertext_size(mlkem_params: MLKEMParams) -> tuple[int, int, int]:
    """
    Compute ML-KEM secret key, public key, and ciphertext sizes with compression.

    Calculates the byte sizes for ML-KEM using the compression parameters (du, dv)
    defined in the scheme. The public key includes a 32-byte seed.
    ...
    Examples:
        ...
        >>> sk, pk, ct = compute_mlkem_key_ciphertext_size(MLKEM512_PARAMS)
        >>> print(f"Secret Key: {sk} bytes")
        >>> print(f"Public Key: {pk} bytes")
        >>> print(f"Ciphertext: {ct} bytes")
    """
    print("=" * 60)
    print("ML-KEM Key and Ciphertext Sizes")
    print("=" * 60)

    # Compute number of bits required to represent the modulus q
    modulus_size_bits = ceil(log2(mlkem_params.q))

    # Secret key: all coefficients of k polynomials, each of degree d
    sk_size_bytes = (mlkem_params.k * mlkem_params.d * modulus_size_bits) // 8

    # Public key: all coefficients of k polynomials + 32-byte seed
    pk_size_bytes = (mlkem_params.k * mlkem_params.d * modulus_size_bits) // 8 + 32

    # Ciphertext with compression: du*k compressed u vector + dv compressed v vector
    ciphertext_size_bytes = int(
        mlkem_params.d / 8 * (mlkem_params.du * mlkem_params.k + mlkem_params.dv)
    )

    return sk_size_bytes, pk_size_bytes, ciphertext_size_bytes


def compute_mlkem_key_ciphertext_size_no_compression(
        mlkem_params: MLKEMParams,
) -> tuple[int, int, int]:
    """
    Compute ML-KEM secret key, public key, and ciphertext sizes without compression.

    Calculates the byte sizes for ML-KEM where all coefficients are stored at full
    modulus_bits precision (no compression applied). The public key includes a 32-byte seed.
    ...
    Examples:
        >>> from scheme_analysis.param_set import MLKEM512_PARAMS
        >>> sk, pk, ct = compute_mlkem_key_ciphertext_size_no_compression(MLKEM512_PARAMS)
        >>> print(f"Secret Key: {sk} bytes")
        >>> print(f"Public Key: {pk} bytes")
    ...
    ciphertext_size_bytes = int(
        mlkem_params.d / 8 * (modulus_size_bits * mlkem_params.k + modulus_size_bits)
    )
