...
//!
//! NOTE (modulus change q = 687659009): coefficients are 30-bit, so the
//! *uncompressed* t / dk_pke rings are packed at 30 bits/coefficient. The
//! per-ring bit count 256*30 = 7680 is byte-aligned at the ring level
//! (= 960 bytes per ring), even though individual coefficients are not.
//! Length constants:
...
//! Reminder: every place below that mentions 30 bits / 960 bytes is tied to
//! params::Q being a 30-bit prime. If Q changes width, update these together
//! (the byte_encode::<30> width and the 960 / 1920 constants).
use crate::ring::*;
use crate::mlkem::*;
use bitvec::prelude::*;
use bitvec::access::*;
